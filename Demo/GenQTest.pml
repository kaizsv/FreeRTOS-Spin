...
do
::  AWAIT_D(_PID, ulLoopCounterSnapshot = ulLoopCounter);
    xQueueSendToFront_NB(xQUEUE, ulLoopCounterSnapshot, intsemNO_BLOCK, _, local_bit, local_xIsTimeOut, local_var1, local_var2, _PID);

    AWAIT_A(_PID, ulLoopCounterSnapshot = 0; assert(uxQueueMessagesWaiting(xQUEUE) == 1));
    ...
    AWAIT_D(_PID, assert(ulLoopCounter == ulData && uxQueueMessagesWaiting(xQUEUE) == 0);
        ulData = 0);
    ...
    AWAIT_A(_PID, ulData = 0; assert(uxQueueMessagesWaiting(xQUEUE) == 3));
    ...
    AWAIT_A(_PID, assert(uxQueueMessagesWaiting(xQUEUE) == 5));
    ...
    AWAIT_A(_PID, ulData = 0; assert(uxQueueMessagesWaiting(xQUEUE) == 0));
    ...
    AWAIT_A(_PID, assert(uxQueueMessagesWaiting(xQUEUE) == 2));
    ...
    AWAIT_A(_PID, ulData = 0; assert(uxQueueMessagesWaiting(xQUEUE) == 5));
    ...
    AWAIT_D(_PID, ulLoopCounter = INC_VAR_AND_WRAP_AROUND(ulLoopCounter));
od
}
...
inline prvTakeTwoMutexesReturnInDifferentOrder(_id, xMutex, xLocalMutex, xReturn, temp_bool, temp_xIsTimeOut, temp_var1, temp_var2, temp_var3)
{
    ...
    AWAIT_D(_id, ulGuardedVariable = 0);

    AWAIT_D(_id, assert(uxTaskPriorityGet(NULL_byte) == genqMUTEX_LOW_PRIORITY));
    ...
    AWAIT_D(_id, assert(uxTaskPriorityGet(NULL_byte) == genqMUTEX_HIGH_PRIORITY));
    ...
    AWAIT_D(_id, assert(ulGuardedVariable == 0));
    ...
    AWAIT_D(_id, assert(ulGuardedVariable == 1));

    AWAIT_D(_id, assert(uxTaskPriorityGet(NULL_byte) == genqMUTEX_TEST_PRIORITY));
...
::  vTaskSuspend(_PID, NULL_byte, local_var1, local_var2);
    AWAIT_D(_PID, ulGuardedVariable = INC_VAR_AND_WRAP_AROUND(ulGuardedVariable));
od
}
