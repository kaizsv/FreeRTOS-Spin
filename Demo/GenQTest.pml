...
::  AWAIT(_PID, ulLoopCounterSnapshot = ulLoopCounter);
    xQueueSendToFront_NB(xQUEUE, ulLoopCounterSnapshot, intsemNO_BLOCK, _, local_var1, local_var2, _PID);
    ...
    AWAIT(_PID, ulLoopCounterSnapshot = ulLoopCounter);
    xQueueSendToBack_NB(xQUEUE, ulLoopCounterSnapshot, intsemNO_BLOCK, _, local_var1, local_var2, _PID);
    ...
    for (ulData: 2 .. 4) {
        xQueueSendToBack_NB(xQUEUE, ulData, intsemNO_BLOCK, _, local_var1, local_var2, _PID);
    }

    AWAIT(_PID, ulData = 0; assert(uxQueueMessagesWaiting(xQUEUE) == 3));

    xQueueSendToFront_NB(xQUEUE, 1, intsemNO_BLOCK, _, local_var1, local_var2, _PID);
    xQueueSendToFront_NB(xQUEUE, 0, intsemNO_BLOCK, _, local_var1, local_var2, _PID);

    AWAIT(_PID, assert(uxQueueMessagesWaiting(xQUEUE) == 5));

    xQueueSendToFront_NB(xQUEUE, 0, intsemNO_BLOCK, local_xReturn, local_var1, local_var2, _PID);
    AWAIT(_PID, assert(local_xReturn == false));

    xQueueSendToBack_NB(xQUEUE, 0, intsemNO_BLOCK, local_xReturn, local_var1, local_var2, _PID);
    AWAIT(_PID, assert(local_xReturn == false));
    ...
    for (ulData: 10 .. 11) {
        xQueueSendToBack_NB(xQUEUE, ulData, intsemNO_BLOCK, local_xReturn, local_var1, local_var2, _PID);
        AWAIT(_PID, assert(local_xReturn == true); local_xReturn = false);
    }
    ...
    for (ulData: 0 .. 2) {
        xQueueSendToFront_NB(xQUEUE, 9 - ulData, intsemNO_BLOCK, local_xReturn, local_var1, local_var2, _PID);
        AWAIT(_PID, assert(local_xReturn == true); local_xReturn = false);
    }
...
inline prvTakeTwoMutexesReturnInDifferentOrder(_id, xMutex, xLocalMutex, xReturn, temp_bool, temp_var1, temp_var2)
{
    ...
    xSemaphoreGive(xMutex, xReturn, temp_var1, temp_var2, _id);
    AWAIT(_id, assert(xReturn == true); xReturn = false);
    ...
    xSemaphoreGive(xLocalMutex, xReturn, temp_var1, temp_var2, _id);
    AWAIT(_id, assert(xReturn == true); xReturn = false);
...
::  vTaskSuspend(_PID, NULL_byte, local_var1);
    ...
    xSemaphoreGive(xMUTEX, local_xReturn, local_var1, local_var2, _PID);
    AWAIT(_PID, assert(local_xReturn == true); local_xReturn = false);
od
}
...
init
{
    ...
    xSemaphoreCreateMutex(xMUTEX, 1, local_var1, local_var2, EP);
    xSemaphoreCreateMutex(xLOCALMUTEX, 2, local_var1, local_var2, EP);
    skip;
